...
void geoLocation::geoLocation(char *jsonPayload) {
    // initialize curl
    ...
    if (curl) {
        ...
        if (res != CURLE_OK) cerr << "ERROR: curl_easy_perform() failed: " << curl_easy_strerror(res) << endl;
        else printLocationInfo(chunk.memory);  // chunk.memory points to a memory block that is chunk.size bytes big and contains the response
    ...
    else cerr << "ERROR: curl_easy_init() failed" << endl;
}

void geoLocation::printLocationInfo(char *response) {
    ...
    if (parsedJSON == NULL) {
        ...
        if (errorPtr != NULL) cerr << "ERROR: cJSON_GetErrorPty returned: " << errorPtr << endl;
        goto end;
    }
...
size_t geoLocation::writeMemoryCallback(void *contents, size_t size, size_t nmemb, void *userp) {
